...
export type FormatStatus = "rewritten" | "unchanged";
...
export function parseFormatJson(raw: string): FormatPayload {
  if (!raw) {
  ...
  if (startIdx === -1) {
    throw new FormatParseError(
      `missing ${FORMAT_START} sentinel in agent output (length=${raw.length})`,
    );
  }
...
function validateFormatPayload(value: unknown): FormatPayload {
  if (!isObject(value)) {
